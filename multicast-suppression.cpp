...
_FIFO::fifo_handler(const std::string& content)
{
  ...
  int bytesRead = read(pipeFile, buffer, sizeof(buffer) - 1);
  buffer[bytesRead] = '\0';
  NFD_LOG_INFO("Python Test: " << buffer);
  NFD_LOG_INFO("Counter " << counter);
...
EMAMeasurements::updateDelayTime(bool wasForwarded, std::string name)
{
  ...
  */
  ...
  NFD_LOG_INFO("The from python = " << from_python);
...
//    m_interestHistory[ndn::Name("Key3")] = {3, true, 3};
    ...
    if (it == m_interestHistory.end()) // the element with key 'name' is not found
    {
      ...
      m_interestHistory[ndn::Name(name)] = {1, isForwarded, send_time_milliseconds};
  ...
  NFD_LOG_INFO("****************** RecordData Function called ********************");
    ...
    if (it == m_dataHistory.end()) // if data is not in dataHistory
    {
      ...
      if(interestRecord != m_interestHistory.end()){
          ...
          NFD_LOG_INFO("name of corresponding interest to get the send time" << name.toUri() << " send time = " << i_time_milliseconds);
          ...
          m_dataHistory[ndn::Name(name)] = {1, isForwarded, receive_time_milliseconds,rtt};
//          it->second.rtt = rtt;
